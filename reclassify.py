...
def load_category_mapping():
    """加载频道分类映射 - 与process_live_sources.py保持一致"""
    # 完整的分类映射（按照您要求的顺序）
    CATEGORY_MAPPING = {
    "央视频道,#genre#":[
...
def normalize_channel_name(channel_name, channel_name_mapping):
    """标准化频道名称 - 与process_live_sources.py保持一致"""
...
def generate_reclassify_file(classified_channels, output_file):
    """生成reclassify.txt文件 - 与reclassified_live_sources.txt格式一致"""
    ...
    output_lines = [
        "# 直播源重新分类结果",
        f"# 生成时间: {beijing_time} (北京时间)",
        f"# 数据来源: https://github.com/q1017673817/iptvz/blob/main/zubo_all.txt",
        ""
    ]
    ...
    for category in category_order:
        if category in classified_channels and classified_channels[category]:
            ...
            for channel_line in classified_channels[category]:
